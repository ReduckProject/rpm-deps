...
# -*- coding: utf-8 -*-
"""
麒麟(Kylin) V10 软件包管理工具
从麒麟官方仓库抓取软件包信息，支持更新元数据和下载rpm包

使用说明:
  python ky_rpm.py --arch=x86_64 --update
  python ky_rpm.py --arch=x86_64 --download=zlib
  python ky_rpm.py --arch=x86_64 --download=zlib --update
"""
...
# 仓库类型
REPO_TYPES = ["base", "updates", "addons"]
...
class KylinPackageFinder:
    ...
    def fetch_packages_from_repo(self, sp_version: str, repo_type: str, repo_url: str) -> List[PackageInfo]:
        ...
        # 麒麟仓库HTML格式: <a href="xxx.rpm">xxx.rpm</a>  date  size
        # 示例: <a href="CUnit-2.1.3-22.ky10.x86_64.rpm">CUnit-2.1.3-22.ky10.x86_64.rpm</a>                     20-Dec-2022 06:37     49K
        # 匹配链接、日期和大小
        pattern = re.compile(
            r'<a[^>]+href="([^"]+\.rpm)"[^>]*>[^<]*</a>\s+(\d{2}-\w{3}-\d{4}\s+\d{2}:\d{2})\s+([\dKMGTPE\.]+)',
            re.IGNORECASE
        )

        for match in pattern.finditer(html):
            package_name = match.group(1)
            package_time = match.group(2).strip()
            package_size = match.group(3).strip()
        ...
        # 如果上面模式没匹配到，尝试简单模式
        if not packages:
            simple_pattern = re.compile(r'<a[^>]+href="([^"]+\.rpm)"[^>]*>', re.IGNORECASE)
            for match in simple_pattern.finditer(html):
                ...
                try:
                    pkg_info = PackageInfo.parse(
                        package_name, sp_version, repo_type, repo_url, "", ""
                    )
                    packages.append(pkg_info)
                except Exception:
                    pass
    ...
    def fetch_all_packages(self):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            for sp_version in SP_VERSIONS:
                # 构建仓库URL
                # 格式: https://update.cs2c.com.cn/NS/V10/V10SP3/os/adv/lic/base/x86_64/Packages/
                for repo_type in REPO_TYPES:
                    url = f"{BASE_URL}/{sp_version}/os/adv/lic/{repo_type}/{self.arch}/Packages/"
                    tasks.append(
                        executor.submit(
...
def main():
    parser = argparse.ArgumentParser(
        description="麒麟(Kylin) V10 软件包管理工具",
...
示例:
  ...
  %(prog)s --download=zlib                     # 使用默认架构(x86_64)下载
        """
    )
    ...
    print("=== 麒麟(Kylin) V10 软件包管理工具 ===")
    print(f"架构: {args.arch}")
    print(f"时间: {datetime.now().isoformat()}\n")

    finder = KylinPackageFinder(arch=args.arch, max_workers=10)
